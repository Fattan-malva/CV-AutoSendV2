...
export async function POST(req: NextRequest) {
  try {
    const { imageData, mimeType: reqMimeType, uid, senderName, language } = await req.json()
    ...
    const result = await analyzeBrochure(imageData, mimeType, senderName || 'Pelamar', language || 'id')
